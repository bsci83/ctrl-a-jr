...
def register_slack_tools(registry: Registry, client: SlackClient) -> None:
    ...
    registry.register(ToolSpec(
        ...
        description="Post a note to the shop's Slack channel. Use it to flag a high-value "
                    "job after the customer's quote has been sent, AND whenever the "
                    "operator simply asks you to post or send something to Slack — you "
                    "can do that, it is not limited to escalations. You do not choose "
                    "the channel; it is configured by the operator, and you cannot DM a "
                    "person or look anyone up by email. Write only the message text.",
        schema={"type": "object",
